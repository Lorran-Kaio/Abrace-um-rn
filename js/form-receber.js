...
document.getElementById("form-receber").addEventListener("submit", async function (event) {
  ...
  try {
    ...
    if (isCpfDuplicate) {
      form.reset();
      alert("Já identificamos uma solicitação com esse CPF. Caso tenha alguma dúvida ou precise atualizar suas informações, entre em contato conosco."); // Trocar pelo toast se aprovado
      sendBtn.disabled = false;
      sendBtn.textContent = "Enviar";
      return;
    }
    // const response = await fetch("https://sheetdb.io/api/v1/j7z1tsiircwdc", {
    const response = await fetch("https://httpbin.org/post", { // Rota de teste, trocar pela de cima apos o teste
      method: "POST",
